...
해당 데이터는 KOKU Airline 내부 예약 데이터와 별도로 관리합니다.
...
지원 공항 및 노선은 개발 과정에서 필요에 따라 일부 조정할 수 있습니다.
...
Google OAuth 로그인을 처음 이용하는 사용자의 경우 필요한 회원 정보를 기반으로 서비스 회원 계정을 생성할 수 있어야 합니다.

일반 로그인과 Google OAuth 로그인 모두 동일한 서비스 회원 및 권한 체계에서 처리되어야 합니다.
...
사용자는 항공편과 좌석을 선택하고 탑승객 정보를 입력하여 예약할 수 있어야 합니다.
...
관리자는 항공편 운영에 필요한 기본 데이터를 관리할 수 있어야 합니다.
...
사용자는 자연어를 통해 항공편 검색 조건을 입력하고 적합한 항공편을 추천받을 수 있어야 합니다.
...
AI는 항공편 번호, 가격, 출발 시간 등의 실제 데이터를 임의 생성하지 않습니다.
...
- [ ] OAuth 2.0 소셜 로그인이 정상 동작합니다.
- [ ] 일반 로그인과 소셜 로그인이 동일한 회원/권한 체계로 처리됩니다.
...
예약, 좌석, 결제 등 핵심 Domain의 규칙은 구현 코드보다 먼저 정의합니다.
...
Portfolio를 위해 기술을 무조건 추가하지 않습니다.
