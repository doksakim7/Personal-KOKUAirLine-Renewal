...
`ACTIVE` 상태의 인증된 Member는 유효한 항공편의 예약 가능한 좌석을 선택하여
`PENDING` 상태의 예약을 시작할 수 있어야 합니다.

예약 시작 시 선택한 좌석은 `HELD` 상태로 전환되어야 합니다.

Member는 탑승객 및 필요한 여권 정보를 입력하고
Mock 결제가 정상적으로 완료된 경우 예약을 `CONFIRMED` 상태로 확정할 수 있어야 합니다.
